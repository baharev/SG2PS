	...
	while  (!(xyfile.eof())) {
		...
		getline (xyfile, ID);
			...
			while  (i < ID.length()) {
				...
				if ((i+1 == ID.length()) && (tabnumber < 2) && (ID.length() > 2)) {
...
vector <LOC_X_Y> competeXYdatabase (string xyname){
	...
	while (!(xyfile.eof())) {
		...
		getline (xyfile, LOC, '\t');
		getline (xyfile, Xtemp, '\t');
		getline (xyfile, Ytemp);

		buffer.LOC = LOC;
		buffer.X = atof(Xtemp.c_str());
		buffer.Y = atof(Ytemp.c_str());

		XY_DB.push_back(buffer);
	...
	cout << "  - Coordinate input completed for " << i << " records." << endl;
